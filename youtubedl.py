...
def downloader(name):
    ...
    try:
        ...
        try:
            ...
            audio_file = eyed3.load(new_filename)
            audio_file.tag.album_artist = links[0]['channel']
            audio_file.tag.title = yt.title
            res = requests.get(links[0]["thumbnails"][0])
            audio_file.tag.images.set(3, res.content, "", u"")
            audio_file.tag.save()
        except Exception:
            print(colors.red + "FFMPEG error!\n", colors.reset)
    except Exception:
        print(colors.yellow + "Song not available on stream servers!\n", colors.reset)
        ...
        try:
            ...
            for char in char_to_ignore:
                ...
                try:
                    ...
                    audio_file = eyed3.load(new_filename)
                    audio_file.tag.album_artist = links[0]['channel']
                    audio_file.tag.title = video.title
                    res = requests.get(links[0]["thumbnails"][0])
                    audio_file.tag.images.set(3, res.content, "", u"")
                    audio_file.tag.save()
                except Exception:
                ...
                for song_first in soup.find_all('span', {'class': 'track-name'}):
                    ...
                    for song_last in soup.find_all('span', {'class': 'artists-albums'}):
                        try:
                            ...
                            pass
                    for song_create in soup.find_all('span', {'class': r"creator-name"'\\'}):
                        try:
                            songs_last_l.append(song_create.text.strip())
                        except Exception:
                            pass
